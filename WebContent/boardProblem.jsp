		...
		<c:choose>
			...
			<c:when test="${sessionScope.loginId != null}">
				<table class="loginTable">
					...
					<button
						onclick="location.href='memberView?kmId=${sessionScope.loginId}'" class="defaultButton">나의
						회원 정보</button>
					<button onclick="location.href='memberLogout'" class="logoutButton">로그아웃</button>
				</table>
			</c:when>
			<c:otherwise>
				<form action="memberLogin" method="POST">
					<table class="loginTable">
						<tr>
							<td colspan="2"><input type="text" name="kmId" placeholder="아이디" ></td>
						</tr>
						<tr>
							<td colspan="2"><input type="password" name="kmPw" placeholder="비밀번호" ></td>
						</tr>
						<tr>
							<td><input type="submit" value="로그인"
								style='cursor: pointer' class="defaultButton"></td>
							<td><button onclick="Join()" style='cursor: pointer' class="joinButton" >회원가입</button><td>
						</tr>
					</table>
				</form>
			</c:otherwise>
